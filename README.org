...
- Our sub-group focused on Instagram.
...
*** Mordecai
- You have to have run Python 3.7
- You have to run the docker command (see [[https://github.com/openeventdata/mordecai][Mordecai]]) from the right directory
- Because the loading of the geoparser from Mordecai, and the set-up of the Geoparsers() take quite some time, it might be useful to debug in ipython (or something like that), in which you can load those elements, and tinker with the code separately.
*** Kepler Python API
...
- The documentation of the config settings in Python are (as far as I know) very limited.
